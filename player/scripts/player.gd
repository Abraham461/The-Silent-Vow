...
#var nav_manager = load("res://player/scripts/navigation_manager.gd").new()
...
var _dialogue_triggered := false   # CHANGED: Flag to prevent repeated triggers
func _ready():
	NavigationManager.on_trigger_player_spawn.connect(_on_spawn)
	animation_player.connect("animation_finished", Callable(self, "_on_animation_finished"))
	# --- Added connections for actionable auto-trigger ---
	actionable_finder.connect("area_entered", Callable(self, "_on_actionable_entered"))   # CHANGED
	actionable_finder.connect("area_exited",  Callable(self, "_on_actionable_exited"))    # CHANGED
...
func _on_actionable_exited(area: Area2D) -> void:    # CHANGED
	if area.has_method("action"):
