...
var textures = [
	preload("res://more cutscene/image/black_obsidian_mountain.png"),
	preload("res://more cutscene/image/tall_tower.png"),
	preload("res://more cutscene/image/caster_ritual.png")
]

var sfx_map = {
	0: preload("res://more cutscene/soundeff/nosoundeff.ogg"),
	1: preload("res://more cutscene/soundeff/lightning_sound_effect(256k).ogg"),
	2: preload("res://more cutscene/soundeff/nosoundeff.ogg")
}
...
func _ready():
	...
	#bgm_player.play()  
	hide_textbox()
	queue_text("At the edge of the world stands Blackreach Keep, carved into a mountain cliff, black as obsidian and older than any map dares record (no map or writen record even tries to show its age or location).")
	queue_image(textures[0])
	queue_text(" Lightning flashes again and again above its tall towers. ")
	queue_image(textures[1])
	queue_text(" Inside, the cult called the Hollow Star prepares for their final ritual. They plan to wake a powerful ancient force. To succeed, they need a royal sacrifice.")
	...
	match current_state:
		State.READY:
			if !text_queue.is_empty():
				...
				get_tree().change_scene_to_file("res://chapter_3.tscn")
		State.READING:
