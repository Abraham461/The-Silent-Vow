...
@onready var demon_health: Node = $SlimeDemonHealth

@onready var hurt_box: Area2D = $HurtBox
@onready var hit_box: Area2D = $HitBox         
	   # should have demonHitBox.gd attached

@onready var demonaggrozone: Area2D = $SlimeDemonAggro
@onready var demonattackzone: Area2D = $SlimeDemonAtk

@onready var sprite: AnimatedSprite2D = $SlimeDemon
...
# --- config ---
@export var attack_anim_name: String = "SlimeDemonAtk"
@export var ATTACK_HIT_FRAMES: Array = [9,10]
...
@onready var firAura: AnimatedSprite2D = $AnimatedSprite2D
...
func _physics_process(delta: float) -> void:
	...
	match state:
		State.PATROL:
			_patrol_timer += delta
			_play_directional_anim("SlimeDemonWalk", facing_right)
